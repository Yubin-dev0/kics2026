...
| netem limit | per base RTT: 640 / 710 / 830 / 980 / 1350 packets for 10 / 30 / 60 / 100 / 200 ms | A8 (9/23): with the default 10000 an 84M L1 filled the FIFO to about 1.6 s of delay and never drained (A8 run 8). The limit now bounds the L1 queue at about 340 ms: 600 packets plus 7500 pps x the one-way delay, fitted on A8 runs 10-11 (`load/README.md`). The first loaded C3 run at every RTT showed the same 335-345 ms rise (`data/c3/NOTES.md`), so the five values are confirmed. `nq <one-way ms> <limit>` sets delay and limit on both interfaces | confirmed 9/23 (A8 for 10 ms, C3 for the rest) |
...
| A4-4 | N5 associates and pings N1 | plan 8 | pass 9/23: N5 on kics-n3 as 192.168.60.55 (5180 MHz); N1 to N5 ping 20/20, 4.129 / 6.819 / 30.537 ms min / avg / max; N5 to N3 20/20, 3.843 / 6.578 / 6.980 ms |
...
| A6-3 | Each run's meta file shows netem on both wlan0 and eth0 with half the set value | the delay is split as plan 5.1 says, not put on one side | pass 9/23: `netem.sh 60` then `netem.sh show` (`data/a6/netem_show_60.txt`) lists wlan0 (handle 8001) and eth0 (8002) each with delay 30 ms limit 10000; after `off` both are back on fq_codel. Not recorded on 9/22 because `tc` was not on the ssh PATH (fixed in `ping_run.py`) |
...
| A5-4 | After installing WireGuard on N1, one A3 bench run (busid 2-3) meets A3-1 to A3-7 | a new network driver on N1 must not disturb the USB path | pass 9/23, A3 run 8 (tunnel on, ping 10.99.0.4 8.0 ms): round trip median 2.793 / p99 4.554 / max 4.932 ms, lost 0 of 2009, C median 34 max 35 us, watchdog gap 149.4 ms. Run 7 (tunnel off, fresh WSL) as reference: 2.649 / 4.285 / 4.792 ms; the tunnel adds about 0.14 ms median, 0.27 ms p99 |
...
A5-4 on 9/22 could not run: `/dev/ttyACM0` appeared after `usbipd attach` and vanished
within a minute; dmesg showed `vhci_hcd: urb->status -104` every ~5 s, then `device
descriptor read/8, error -110` and `USB disconnect`, and `usbipd attach` finally hung.
WSL had been up for 6 h. On 9/23 with a fresh WSL both runs passed. Their dmesg has
`-104` lines too (17 in run 7, 34 in run 8), but all in one 6 ms burst per port close
(unlink of the pending URBs), with no `-110`, no disconnect and the device still present:
not the 9/22 pattern. The 9/22 drop is not isolated; a WSL restart before a session is
the working rule. (The N1-N2 USB cable was later found loose and replaced before B3 run
4, which may be the 9/22 cause.)
...
| ID | criterion | basis | status |
|---|---|---|---|
| A9-1 | 100 UDP exchanges N1 to N3 (`clock_offset.py`), standard deviation of the per-exchange offset under 1 ms | plan v5 7.3 | pass 9/23, run 2: delta -54174117.577 ms, sd 0.176 ms, best-quarter median 0.045 ms from the full median, round trip median 1.348 / max 4.702 ms. Run 1 FAIL (sd 1.759 ms, a 27.3 ms Wi-Fi spike) and repeated under the burst rule |
| A9-2 | one `clock_run_N.json` next to every sweep run, taken right before it | the Pi 5 has no RTC; D and B both carry delta, so a stale delta moves both. N3 runs about 15 h behind N1 and drifts about 0.13 ms per minute, so a per-run file is needed | pass: C1 runs 1-3 and C3 runs 1-69 each have one (70 files in `data/c3/`, run 0 being the pre-sweep check). A FAIL (sd over 1 ms, a Wi-Fi burst) was repeated at once: C3 run 0 once, runs 19 (twice), 21 and 40 once (`data/c3/NOTES.md`) |
...
- N3 is the Pi 5 (received 2026-09-22); a rented Pi 4 is N5 and the other a spare. A
  ...
  moves to the spare Pi 4 and A4, A6 and A5 are run again with the same scripts. N3 was
  not changed from A4 through C3 (all on 9/22-9/23).
